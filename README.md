...
- **Content_ids format fix**: `content_ids: [variantId]` — ТОЛЬКО variant_id для 95-100% Match Rate
...
1. Откройте `/dist/pl/pixel.v16.pl.FULL.html`
...
- Аналогично, используйте `/dist/ro/pixel.v16.ro.FULL.html`
...
### Текущие результаты:
- **PL:** Match Rate 95-100% (65 parents, 109 variants)
- **DE:** Match Rate 95-100% (51 parents, 80 variants)
- **RO:** Match Rate 95-100% (65 parents, 108 variants)

### Почему формат [variantId] важен:
- Meta каталог содержит ТОЛЬКО variant_id в колонке `id`
- Отправка только variant_id даёт 95-100% match
- Group ID нужен только в самом каталоге для группировки вариантов
